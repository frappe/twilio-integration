...
def voice(**kwargs):
	try:
		default_outgoing = frappe.db.get_single_value("Twilio Settings", "default_outgoing_medium")
		...
		if args.To != '':
			...
			phone = args.To
			dial = Dial(caller_id=default_outgoing)
...
def update_call_log(call_sid, status="In Progress"):
	...
	frappe.db.set_value("Call Log", call_sid, "status", status)